"""
Modified from Point Transformer V2 by Xiaoyang Wu (xiaoyang.wu.cs@gmail.com)
...
from utils.utils import offset2batch, batch2offset
...
class BatchScatterPool(nn.Module):
    def __init__(self, offset, type) -> None:
        super().__init__()
...
class GroupedVectorAttention(nn.Module):
    ...
    def forward(self, feat, coord, reference_index):
        # inference_index is from pointops.knn_query(self.neighbours, coord, offset)
        query, key, value = (
            self.linear_q(feat),
            self.linear_k(feat),
            self.linear_v(feat),
        ...
        pos, key = key[:, :, 0:3], key[:, :, 3:]  # (n, ns, c)
        relation_qk = key - query.unsqueeze(1)  # (n, ns, c) - (n, 1, c) -> (n, ns, c)
        if self.pe_multiplier:
        ...
        weight = self.weight_encoding(relation_qk)   # (n, ns, group)
        weight = self.attn_drop(self.softmax(weight))  

        mask = torch.sign(reference_index + 1)  # (n, ns) 值全都变为+1 这意义是什么？？
        weight = torch.einsum("n s g, n s -> n s g", weight, mask)  # 这行中，只有相乘发生，没有相加
        value = einops.rearrange(value, "n ns (g i) -> n ns g i", g=self.groups)  # 把value的特征维度拆为group个组
...
class BlockSequence(nn.Module):
    ...
    def forward(self, points):
        ...
        # all blocks in one stage uses the same reference_index
...
class GridPool(nn.Module):
    ...
    def forward(self, points, start=None):
        ...
        # compute the min limit of (x,y,z) for each sample in the batch
        # (B, 3)
        start = (
            segment_csr(
                coord,
        ...
        # cluster indicates which grid each point is in
        cluster = voxel_grid(
        ...
        # all the xyz are aggregated by mean
        coord = segment_csr(coord[sorted_cluster_indices], idx_ptr, reduce="mean")
        # all the features are aggregated by max
        feat = segment_csr(feat[sorted_cluster_indices], idx_ptr, reduce="max")
        batch = batch[idx_ptr[:-1]]
        offset = batch2offset(batch)
        return [coord, feat, offset], cluster  # cluster (n,1), indicate grid id of each point
...
class PointTransformerV2(nn.Module):
    def __init__(
        ...
        for i in range(self.num_stages):
            ...
            assert global_pool_method in ['max', 'mean']
    ...
    def forward(self, data_dict):
        ...
        skips = [[points]]   # [[points, cluster], [points, cluster]]
        for i in range(self.num_stages):
        ...
        direct_out = points  # encoder-level direct output

        for i in reversed(range(self.num_stages)):
            skip_points, cluster = skips.pop(-1)
            points = self.dec_stages[i](points, skip_points, cluster)
        coord, feat, offset = points   # decoder-level poiont-wise output

        return direct_out, feat
